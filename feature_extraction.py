...
def tokenize(reviews, nb_tokens=100):
    ...
    new_sw = ['phone', 'wa', 'work', 'month', 'year', 'day',
              'iphone', 'time', 'ha', 'way', 'wife', 'week',
              'ratings', 'money', 'doe', 'amazon', 'htc', 'rating']
...
def get_filter_token(text, common):
    ...
    wordnet_lemmatizer = nltk.stem.WordNetLemmatizer()
    lemmatized_tokens = [wordnet_lemmatizer.lemmatize(
        t) for t in no_stops_tokens]
...
def get_cmn_nlp_features(df, cmn_nn, cmn_adj):
    ...
    df['tokens_cmn_nn'] = df.apply(
        lambda row: get_filter_token(row['reviewText'], cmn_nn), axis=1)
    df['tokens_cmn_adj'] = df.apply(
        lambda row: get_filter_token(row['reviewText'], cmn_adj), axis=1)
    df['sents_length'] = df.apply(lambda row: len(row['tokens']), axis=1)
    df['tokens_pos'] = df.apply(lambda row: get_tag(row['tokens']), axis=1)
    df['aspects'] = df.apply(
        lambda row: get_aspects(row['tokens_pos']), axis=1)
    return df
